...
//		add_meta_box( 'post_settings_sectionid', __( 'Settings', 'schloss' ), 'post_settings_inner_custom_box', array('page'), 'side', 'low' );
	};
}
...
//NOTES
/*
	$dark     = get_post_meta($post->ID, "_dark", true);
	...
	$checked = ($dark) ? " checked=\"checked\"" : "";
	echo "<label><input type=\"checkbox\" name=\"dark\" value=\"1\"{$checked} /> Use Dark Title</label><br />";
...
function post_settings_save_postdata( $post_id, $post, $update ) {
	...
	$vars = array("dark", "title_split", "overlap_hide", "margin_none", "bg_fixed");
